...
const EditSupplierOrderModal: React.FC<EditSupplierOrderModalProps> = ({
  isOpen,
  onClose,
  onOrderUpdated,
  order,
  ...
  // Load previous items when supplierId changes
  useEffect(() => {
    if (supplierId > 0) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err.message || 'Error al guardar los cambios de la orden');
    } finally {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 overflow-hidden border border-gray-100 flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-slate-50 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-gray-900">Editar Orden de Proveedor #{order.id}</h2>
              <p className="text-sm text-gray-500">Modifica los detalles y artículos del pedido</p>
            </div>
          </div>
          <Button
            type="button"
